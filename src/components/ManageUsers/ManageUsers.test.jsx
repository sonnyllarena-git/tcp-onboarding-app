...
import ManageUsers, { filterUsers, getPaginatedUsers, getRequestIdByEmail } from './ManageUsers';
...
describe('ManageUsers', () => {
  ...
  it('navigates to the matching pending request when "View Pending Details" is clicked', async () => {
    renderManageUsers();
    fireEvent.click(screen.getByRole('button', { name: 'Actions for Olivia Martin' }));
    fireEvent.click(screen.getByRole('menuitem', { name: 'View Pending Details' }));

    // Olivia Martin (user id 1) maps to request id 101 by email - see
    // getMockRequestByEmail in mockData.js.
    expect(await screen.findByText('Request Details Stub')).toBeInTheDocument();
  });
});

describe('getRequestIdByEmail', () => {
  it('returns the matching request id for a known pending email', () => {
    expect(getRequestIdByEmail('olivia.martin@thecreditpros.com')).toBe(101);
  });

  it('returns null when no request matches the email', () => {
    expect(getRequestIdByEmail('nobody@thecreditpros.com')).toBeNull();
  });
});
